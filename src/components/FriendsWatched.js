...
export const FriendsWatched = () => {
  const [FriendsWatchedMovieData, setFriendsWatchedMovieData] = useState([]);
  const location = useLocation()
  const name = location.state.fromw.Name
     ...
     console.log(res['data']['Watched'].length);
      ...
      for(var key in res['data']['Watched']){
        ...
        const struct = <div>{temp}</div>
         ...
         setFriendsWatchedMovieData(FriendsWatchedMovieData=>[...FriendsWatchedMovieData,
             struct
         ])
...
return (
    <div>
      <h2> {name}'s Watched Movies & Shows</h2>
      <br/>
      <br/>
      <div className='list'>
     
      {FriendsWatchedMovieData}
